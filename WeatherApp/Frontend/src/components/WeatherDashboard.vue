...
<template>
  ...
  <div class="dashboard">
    ...
    <section class="card">
      <div class="btn-card">
        <button class="refresh-btn" @click="props.onRefresh(1)">↺ Задержка</button>
        <button class="refresh-btn" @click="props.onRefresh(2)">↺ Ошибка</button>
      </div>
    </section>
  </div>
...
.cur-icon {
  width: 96px;
  height: 96px;
  filter: drop-shadow(0 0 14px rgba(96, 165, 250, .4));
}
